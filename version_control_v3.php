...
$username = "cosahodiusato";
$password = "'fipbatugto70";
$dbname = "my_cosahodiusato";
...
$est_ultima=false;
  ...
  if($est_prog==true){
		...
		if(($row[0]==$_GET['name'])&&($row[1]==$_GET['version'])){
			echo "-";//riga 4 - se indica un numero vuol dire che è quello dell'ultima versione aggiornata, e che quindi il client sta usando una versione obsoleta
			echo "<br>";
			$est_ultima=true;
			$salva=false;
		}else{
			if($est_version==true){
				...
				echo $row[1];//"-";//riga 4 - se indica il segno meno '-' vuol dire che il client sta usando una versione nuova dell'applicazione oppure è la prima versione di una nuova applicazione
				echo "<br>";
				$salva=false;
		}
	}
